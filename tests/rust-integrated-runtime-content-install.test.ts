...
test("Unicode and high-byte content page matches the frozen Rust wire vector", () => {
  const bundle = compileRustProductionContent("content-\u6c34-1", [{
    ...
    aliases: ["item:orb:\u6c34", "item:orb-\u6c34"], value: { name: "Mizu \u6c34" }, unknownExtensionBytes: Uint8Array.of(0, 0x80, 0xff),
  }]);
  const plan = createRustContentInstallPlanV1(bundle);
  assert.equal(bundle.manifest?.manifestHash, "2bbeaab8b5f3691230252027558e948f");
  assert.equal(
