...
The terrain row remains `typescript-authoritative` in this formal ledger. On 2026-09-04 the checked-in selector names canonical compatibility artifact `ab4d66c5c165779c08bd1098be8b8ff1ad71b5702d938b72f38faae0c81493b1`. Its Wasm payload is `f1b2286e350bf8839e387dad5b03b6ad4e7424e0ef68a9838fad252b5164594f` (7,424,538 bytes), and its current build-only manifest binds source snapshot `d4b466adda4483cca622b917295e07ea74d538be934a627f71d9756af1ecbb8a` / 230 files. Canonical, no-override tests pass all 155 generation cases (131 frozen plus 24 normalized public-option lanes, including `legacy-v14`; corpus `5d4e6b1445b00f3430164d1a8093d8dc`) and all 114 settlement / 16 lair locator cases. The five shared Three/wgpu landscape records pass the exact-source check retained at `work/hybrid-rust-migration/r3-landscape-ab4d66c5-20260904-final-check-1/`; geometry, materials, camera records and generated chunk hashes remain bound by the fixture evidence.

The same publication keeps renderer-lab separate at artifact `988d4425660abdc12b055c0c1fbc082d5870fa7ff6ec2009f68d7c0e5a9c33fd`, nested-manifest SHA-256 `48e4d3a3de191689fdee6b445347ff49d8bc3eb2a9c29060f6603f154f4ebb9b`, and Wasm `5f188f05bec036ccbc44fd2e24bf70a483c5cf37cafdd22a6edd2b1e96de80e9` (7,552,651 bytes), from the same `d4b466...` / 230-file source snapshot. Its browser smoke is a bounded exact-artifact/offscreen-WebGPU gate only; the final retained passing evidence path is root-owned and pending insertion, and the placeholder does not itself claim a passing run. None of this is a deployment or formal authority/renderer promotion.
...
The accepted five-pair subsystem rerun is a distinct provenance record: committed tree `e19553f` used source snapshot `9a6cc555af118f957456eea452a49a967c07f54a544fa8187c14535b66cd5212` / 230 files and reproduced compatibility bytes `ab4d66c5...`. Across five alternating pairs, complete-corpus initialized p95 was 202.505 ms for TypeScript and 156.960 ms for Rust (0.7751x); reset-to-accepted was 202.745 ms versus 175.305 ms (0.8647x). Every declared generation and landscape ratio stayed below 1.05, but the retained result intentionally says `performanceAcceptance: false`. The later `d4b466...` build-only source identity in the checked-in manifests must not be read backward into this `e19553f` performance run. Evidence: `work/r3-e19553f-acceptance-1/work/r3-five-pair-3/`.

The current canonical real-worker gate at
`work/hybrid-rust-migration/browser/r3-production-worker-ab4d66c5-20260904-canonical-1/`
passes 155 cases in three orders (465
...
were repeated successfully after the validated byte-hasher optimization. The
final accepted edit-gate evidence path is root-owned and pending insertion here.

The real persistent-cache/POI gate at
`work/hybrid-rust-migration/r3-persistent-cache/ab4d66c5-20260904-canonical-1/`
passes natural eviction, committed IndexedDB
...
edit witness. Both historical public imports preserve their source but are refused
by the existing world-only native migration policy; rich-save adoption remains an
implementation dependency, not an accepted import. The five-pair browser study now
passes its declared subsystem regression ceilings, but its explicit
`performanceAcceptance: false` prevents that evidence from self-promoting R3 or
standing in for whole-game acceptance. The 155-case code parity, canonical binding,
actual-worker correctness/recovery, current landscape check and live edit-halo cache
rejection are no longer open implementation prerequisites. The formal acceptance
count remains 9/32 and the verified rollback-release count remains 0/2.
...
An earlier coherent source/artifact checkpoint was closed by local
...
existing skips, and TypeScript. The newer current publication and browser evidence
above bind the separate build-only `d4b466...` / 230-file source snapshot; that
manifest binding is not itself a new authority-promotion record. No push or
